...
export function useImageRecognition(): ImageRecognitionHook {
  ...
  const recognize = useCallback(
    async (imageFile: File): Promise<RecognitionResult> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const notes = extractNotesFromImageData(
          imageData,
          canvas.width,
          canvas.height,
        );
...
function extractNotesFromImageData(
  imageData: ImageData,
  width: number,
  height: number,
): ScoreNote[] {
  ...
  if (staffLines.length < 2) return generateDemoNotes();
  ...
  for (const notehead of noteheads) {
    ...
    const duration = notehead.filled ? 500 : 1000;
    notes.push({
      pitch,
  ...
  return notes.length === 0 ? generateDemoNotes() : notes;
}
...
function findNoteheads(
    ...
    for (let x = minRadius; x < width - minRadius; x++) {
      ...
      if (blob.pixels.length > 0) {
        ...
        if (radius >= minRadius && radius <= maxRadius) {
          ...
          noteheads.push({ x: cx, y: cy, filled: blob.density > 0.6, radius });
        }
      }
    }
  }
  ...
  for (const nh of noteheads) {
    if (
      !filtered.some(
        (f) => Math.abs(f.x - nh.x) < 15 && Math.abs(f.y - nh.y) < 15,
      )
    )
      filtered.push(nh);
  }
  ...
  const boundingArea =
    pixels.length > 0
      ? (Math.max(...pixels.map((p) => p.x)) -
          ...
          1) *
        (Math.max(...pixels.map((p) => p.y)) -
          Math.min(...pixels.map((p) => p.y)) +
          1)
      : 1;
  return {
    pixels,
...
function mapPositionToPitch(
  ...
  if (staffLines.length < 2)
    return DIATONIC_PITCHES[
      Math.floor(Math.random() * DIATONIC_PITCHES.length)
    ];
  ...
  const clampedIndex = Math.max(
    0,
    Math.min(DIATONIC_PITCHES.length - 1, pitchIndex),
  );
