...
export function useScoreEditor(): ScoreEditorState & ScoreEditorActions {
  ...
  const addNote = useCallback(
    (pitch: string, duration: number) => {
      ...
      setNotes((prev) => {
        const insertAt =
          selectedNoteIndex !== null ? selectedNoteIndex + 1 : prev.length;
        const updated = [...prev];
  ...
  const insertNoteAt = useCallback(
    (index: number, pitch: string, duration: number) => {
      const newNote: ScoreNote = {
        pitch,
        duration: BigInt(duration),
        fingering: [],
  ...
  const updateNote = useCallback(
    (index: number, changes: Partial<ScoreNote>) => {
      setNotes((prev) =>
        prev.map((note, i) => (i === index ? { ...note, ...changes } : note)),
      );
    },
    [],
  );
