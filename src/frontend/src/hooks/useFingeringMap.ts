...
function tupleArrayToMap(
  data: Array<[string, Array<boolean>]>,
): FingeringMap | null {
...
export function useFingeringMap(): UseFingeringMapReturn {
  const [fingeringMap, setFingeringMap] =
    useState<FingeringMap>(buildDefaultMap);
  const [isDirty, setIsDirty] = useState(false);

  // Track whether we've already hydrated from backend to avoid overwriting user edits
  const hydratedRef = useRef(false);

  const { data: backendData, isLoading: isLoadingFromBackend } =
    useLoadFingeringMap();
  ...
  const saveToBackend = useCallback(async () => {
    const tuples: Array<[string, Array<boolean>]> = DIATONIC_NOTES.map(
      (note) => [
        note,
