...
export function useAddPresetSong() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      displayName,
      score,
    }: { id: string; displayName: string; score: Score }) => {
      if (!actor) throw new Error("Actor not ready");
...
export function useGetHolePresets() {
  const { actor, isFetching } = useActor();
  return useQuery<HolePreset[]>({
    queryKey: ["holePresets"],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getHolePresets();
    },
    enabled: !!actor && !isFetching,
  });
}

export function useSaveHolePreset() {
  const { actor } = useActor();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({
      preset,
      password,
    }: { preset: HolePresetForm; password: string }) => {
      if (!actor) throw new Error("Actor not ready");
      return actor.saveHolePreset(preset, password);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["holePresets"] });
    },
  });
}

export function useDeleteHolePreset() {
  const { actor } = useActor();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, password }: { id: string; password: string }) => {
      if (!actor) throw new Error("Actor not ready");
      return actor.deleteHolePreset(id, password);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["holePresets"] });
    },
