  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Fingering map saved!", {
        description:
          "Your custom fingering configuration has been saved to the backend.",
        duration: 3000,
      });
    } catch (err) {
...
const HoleButton: React.FC<HoleButtonProps> = ({ closed, label, onClick }) => (
  <button
    ...
    className={`w-6 h-6 rounded-full border-2 transition-all duration-150 cursor-pointer focus:outline-none focus:ring-2 focus:ring-warm-brown focus:ring-offset-1 ${
      closed
