...
import { CheckCircle, Loader2, Volume2, X } from "lucide-react";
import React, { useState, useCallback } from "react";
import { toast } from "sonner";
import { ExternalBlob } from "../backend";
import {
  useDeleteSampleAssignment,
  useGetAllSampleAssignments,
  useUploadSample,
...
export const SampleUploadPanel: React.FC<SampleUploadPanelProps> = ({
  onSamplesChange,
}) => {
  ...
  React.useEffect(() => {
    ...
    for (const [note, blob] of Object.entries(assignmentMap)) {
          ...
          >
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <label className="cursor-pointer">
                ...
                />
                <span
                  className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-body border border-border bg-card hover:bg-muted transition-colors ${isUploading ? "opacity-50 pointer-events-none" : ""}`}
                >
                  {isUploading ? (
