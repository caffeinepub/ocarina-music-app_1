...
import { type OcarinaProfile, SizePreset } from "../backend";
...
// Badge is imported but used via SIZE_PRESET_INFO color classes - keep for future use
void Badge;
...
export function OcarinaProfileCard({
  ...
  const sizeInfo =
    SIZE_PRESET_INFO[profile.size] ?? SIZE_PRESET_INFO[SizePreset.alto];
  ...
  return (
    <div className="flex-shrink-0 w-52 rounded-xl overflow-hidden border border-border bg-card shadow-sm hover:shadow-md transition-shadow group">
        ...
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src =
              "/assets/generated/fp-hands-ocarina.dim_600x400.png";
          }}
        />
        {showActions && (
      ...
      <div className="p-3 space-y-2">
        ...
        <div
          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-medium ${sizeInfo.color}`}
        >
