...
import { STAFF_POSITIONS, getFingeringForPitch } from "../utils/fingeringMap";
...
function getPitchY(pitch: string): number {
  ...
  return bottomLineY - (staffPos - 2) * (LINE_SPACING / 2);
}
    ...
    >
      ...
      >
        ...
        {Array.from({ length: rows }).map((_, rowIdx) => {
          ...
          const rowNotes = notes.slice(
            rowIdx * NOTES_PER_ROW,
            (rowIdx + 1) * NOTES_PER_ROW,
          );
          return (
            <g key={`row-y${rowY}`} transform={`translate(0, ${rowY})`}>
              {STAFF_LINE_Y.map((lineY) => (
                <line
                  key={`staffline-row${rowIdx}-y${lineY}`}
                  x1={20}
                  y1={lineY}
                  x2={svgWidth - 20}
                  y2={lineY}
              ...
              {rowNotes.map((note, noteIdx) => {
                ...
                return (
                  <g
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && onNoteSelect(globalIdx)
                    }
                    style={{ cursor: "pointer" }}
                    tabIndex={0}
                    aria-label={`Note ${note.pitch}`}
                  >
                    ...
                    <ellipse
                      ...
                      ry={7}
                      fill={
                        filled ? (isPlaying ? "#c8613a" : "#3d2b1f") : "none"
                      }
                      stroke={isPlaying ? "#c8613a" : "#3d2b1f"}
                      strokeWidth={1.5}
                    ...
                    <g
                      transform={`translate(${x - 12}, ${STAFF_LINE_Y[4] + 22})`}
                    >
              ...
              {rowIdx === rows - 1 && onAddNoteAt && (
                <g
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && onAddNoteAt(notes.length)
                  }
                  style={{ cursor: "pointer" }}
                  tabIndex={0}
                  aria-label="Add note"
...
const TablatureDiagramSVG: React.FC<TablatureDiagramSVGProps> = ({
  fingering,
  size,
}) => {
