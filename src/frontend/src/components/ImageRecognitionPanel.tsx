...
export const ImageRecognitionPanel: React.FC<ImageRecognitionPanelProps> = ({
  ...
  const [recognizedNotes, setRecognizedNotes] = useState<ScoreNote[] | null>(
    null,
  );
  ...
  const handleUpdateNote = useCallback(
    (index: number, changes: Partial<ScoreNote>) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <label className="relative border-2 border-dashed border-border rounded-lg p-4 text-center cursor-pointer hover:border-primary transition-colors bg-secondary block">
        <input
          ref={fileInputRef}
          type="file"
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="max-h-40 overflow-y-auto scrollbar-thin flex flex-col gap-1 pr-1">
            {recognizedNotes.map((note, i) => {
              ...
              return (
                <div
                  key={`note-${note.pitch}-${i}`}
                  className="flex items-center gap-2 p-1.5 rounded border border-border bg-card"
                >
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      handleUpdateNote(i, { duration: BigInt(val) })
                    }
                  >
