...
import {
  useBulkCreateOcarinaProfiles,
  useCreateOcarinaProfile,
  useUpdateOcarinaProfile,
...
export function OcarinaProfileForm({
  open,
  onClose,
  editingProfile,
}: OcarinaProfileFormProps) {
  ...
  const [size, setSize] = useState<SizePreset>(
    editingProfile?.size ?? SizePreset.alto,
  );
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          const entryName =
            selectedFiles.length > 1
              ? `${profileData.name} ${i + 1}`
              : profileData.name;
          entries.push([
            generateId(),
            { ...profileData, name: entryName },
            imageBlob,
          ]);
        }
    ...
    <>
      <Dialog open={open} onOpenChange={(o) => !o && !isLoading && onClose()}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {isEditing ? "Edit Ocarina Profile" : "Upload Ocarina Profile"}
            </DialogTitle>
            <DialogDescription>
              {isEditing
          ...
          <div className="space-y-4">
              ...
              <Label htmlFor="profile-name">
                ...
                {selectedFiles.length > 1 && (
                  <span className="text-muted-foreground text-xs">
                    (will be numbered: Name 1, Name 2…)
                  </span>
                )}
              </Label>
              <Input
            ...
            <div className="space-y-2">
              ...
              <Select
                value={size}
                onValueChange={(v) => setSize(v as SizePreset)}
              >
                <SelectTrigger>
              ...
              <p className="text-xs text-muted-foreground">
                Sets the tonal range: Low Bass (C3) · Bass (C4) · Alto (C5) ·
                Soprano (C6)
              </p>
            </div>
            {savingProgress && (
