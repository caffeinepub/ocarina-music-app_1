...
import {
  useDeleteOcarinaProfile,
  useGetOcarinaProfiles,
...
export function OcarinaShowcasePanel({
  onPlayProfile,
}: OcarinaShowcasePanelProps) {
  const [expanded, setExpanded] = useState(true);
  const [formOpen, setFormOpen] = useState(false);
  const [editingProfile, setEditingProfile] = useState<OcarinaProfile | null>(
    null,
  );
            ...
            {isLoading ? (
              <div className="flex gap-4 py-2">
                {[1, 2, 3].map((i) => (
                  <Skeleton
                    key={i}
                    className="flex-shrink-0 w-52 h-64 rounded-xl"
                  />
                ))}
              </div>
            ) : profiles.length === 0 ? (
