...
export const PresetSongLibrary: React.FC<PresetSongLibraryProps> = ({
  onLoad,
}) => {
  ...
  const addPresetSongRef = useRef(addPresetSong);
  addPresetSongRef.current = addPresetSong;

  useEffect(() => {
    if (!presetList || seededRef.current) return;
    ...
    const seed = async () => {
      for (const song of missing) {
        try {
          await addPresetSongRef.current.mutateAsync({
            id: song.id,
